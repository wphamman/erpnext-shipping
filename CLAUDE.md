...
- **TCG Locker (PUDO) L2L**: default-disabled; resolves settings from the FIRST ES instance with `tcg_locker_enabled=yes` (global, via `es_tcg_locker_settings()` / `es_tcg_locker_client()`). Contract authority = the published sandbox Postman collection (`docs/tcg-locker-architecture.md`); sandbox base `https://sandbox.api-pudo.co.za/api/v1`, Bearer auth; label URLs sit OUTSIDE `/api/v1` (origin = base minus `/api/v1`). **L2L only** (collection is `type:locker`, never a source terminal). Pure/testable logic (`rate`, `packer`, `booking`, `tracking`) has a no-WP harness — run `php tests/run.php`. Checkout selector on `woocommerce_review_order_after_shipping`; rate id ends `_locker`; quote snapshot (`_es_tcg_locker_*`) persists onto the order shipping item via WC's rate `meta_data` copy. **Booking is manual, admin-only, idempotent, and never automatic** — capability + per-order nonce + atomic ownership-token mutex (immutable lease) + durable duplicate/in-progress guard + fresh cache-bypass drift refusal + ambiguous-timeout state. Booking outputs: order meta `_es_tcg_locker_booking_status` (`none|booking|booked|ambiguous|error`), `_es_tcg_locker_shipment_id`, `_es_tcg_locker_tracking_ref`, `_es_tcg_locker_booked_ts`. Tracking poll is forward-only (§12 map), authenticated, using the plugin's own token (NOT the Courier-Guy portal token). Rollback = set `tcg_locker_enabled=no` — stops NEW locker quotes/bookings (checkout selector + `_locker` rate self-gate on the client), but the admin panel still shows on orders carrying the `_es_tcg_locker_*` shipping-item snapshot (Book action is then inert) and booked orders stay in the tracking poll (keyed on `booking_status=booked`/shipment_id, not the flag) — so KEEP the token configured until in-flight booked orders are terminal, else the disabled client's polls false-fail into the watchdog. Credentials persist on disable. Full removal = deactivate the plugin (default-disabled). No data destroyed. Full design + status vocabulary in `docs/tcg-locker-architecture.md`; sandbox test steps in `docs/tcg-locker-uat.md`.
