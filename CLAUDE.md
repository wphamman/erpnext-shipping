...
│   ├── class-es-shipping-tax.php            # Pure inclusive shipping tax split
│   ├── class-es-option-mutex.php             # DB-atomic ownership-tokened mutex
...
- **Door carrier booking (v1.14)**: new single-origin TCG/MDS rates persist an `_es_carrier_*` order-shipping-item snapshot including the exact provider booking service and tier. The unified *Carrier Fulfilment* sidebar can retain the customer carrier or override to the other configured carrier's cheapest service in that same tier. The original choice remains immutable; booked-provider meta drives tracking and native PDFs. A read-only live-cost check persists a five-minute confirmation bound to current order facts, then booking re-quotes and requires the exact service/cents before the second action's explicit warning. Booking uses a durable in-progress state + `ES_Option_Mutex`'s unique-index `INSERT IGNORE`; ambiguous responses never auto-retry. Courier Guy create/document calls use the current Portal `/v2` endpoints and current signed-PDF host allowlist. MDS creates with `auto_accept=true` only after confirmation. The cron uses an atomic one-hour immutable lease and reserves pre-shipment slots so Processing can advance on a real carrier event. Legacy and split orders remain portal/manual-tracking only. Staging runbook: `docs/carrier-booking-uat.md`.
- **Shipping tax contract (v1.14.1)**: Courier Guy/MDS quotes are VAT-inclusive customer totals. Before calling `add_rate()`, `ES_Shipping_Method::add_tax_inclusive_rate()` uses WooCommerce's active shipping tax rates to provide an explicit net cost and tax map, so the displayed gross is unchanged and ERP sync receives distinct `shipping_total`/`shipping_tax`. Merchant-entered fallback/own-vehicle prices use `add_configured_rate()` and follow `wc_prices_include_tax()` (gross retail; ex-VAT wholesale). TCG Locker already supplies an ex-VAT cost and deliberately bypasses both helpers.
