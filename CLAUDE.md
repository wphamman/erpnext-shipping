...
- **Shipping tax contract (v1.14.1)**: Courier Guy/MDS quotes and merchant-entered fallback/own-vehicle prices are VAT-inclusive customer totals. Before calling `add_rate()`, `ES_Shipping_Method::add_tax_inclusive_rate()` uses WooCommerce's active shipping tax rates to provide an explicit net cost and tax map, so the displayed gross is unchanged and ERP sync receives distinct `shipping_total`/`shipping_tax`. TCG Locker already supplies an ex-VAT cost and deliberately bypasses this helper.
